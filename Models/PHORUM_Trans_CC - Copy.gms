...
UPTIME1c(g).. sum(t$(ord(t) gt 1 and ord(t) le (gOntime(g)+1)*gInitState(g)), 1 - U(g,t)) =e= 0;
UPTIME2c(g,t)$(ord(t) gt gOntime(g)*gInitState(g)+1 and ord(t) lt card(t)-gMinUp(g)+2).. sum(tp$(ord(tp) ge ord(t) and ord(tp) lt ord(t)+gMinUp(g)),U(g,tp)) =g= gMinUp(g)*(U(g,t) - U(g,t-1));
UPTIME3c(g,t)$(ord(t) ge card(t)-gMinUp(g)+2 and ord(t) lt card(t)).. sum(tp$(ord(tp) gt ord(t)), U(g,tp)) =g= (card(t)-ord(t))*(U(g,t)-U(g,t-1));

* Min downtime (updated mbruchon May 4 2021)
DOWNTIME1c(g).. sum(t$(ord(t) gt 1 and ord(t)le (gDowntime(g)+1)*(1-gInitState(g))), U(g,t)) =e= 0;
DOWNTIME2c(g,t)$(ord(t) gt gDowntime(g)*(1-gInitState(g))+1 and ord(t) lt card(t)-gMinDown(g)+2).. sum(tp$(ord(tp) ge ord(t) and ord(tp) lt ord(t)+gMinDown(g)),1-U(g,tp)) =g= gMinDown(g)*(U(g,t-1) - U(g,t));
DOWNTIME3c(g,t)$(ord(t) ge card(t)-gMinDown(g)+2 and ord(t) lt card(t)).. sum(tp$(ord(tp) gt ord(t)), 1-U(g,tp)) =g= (card(t)-ord(t))*(U(g,t-1)-U(g,t));
...
*execute_unload "results.gdx" gLevel U HourlyCost gVC TI12 TI15 TI13 TI52 TI23 TI34 sDischarge sCharge vSOC.l vCharge.l sSOC SUPPLYTCR1c.m SUPPLYTCR2c.m SUPPLYTCR3c.m SUPPLYTCR4c.m SUPPLYTCR5c.m loadTCR1 loadTCR2 loadTCR3 loadTCR4 TI12max loadTCR5 TI13max TI15max TI52max TI23max TI34max
*execute_unload "results.gdx" gap gLevel U HourlyCost vSOC.l vCharge.l gVC TI12 TI15 TI13 TI52 TI23 TI34 sDischarge sCharge sSOC SUPPLYTCR1c.m SUPPLYTCR2c.m SUPPLYTCR3c.m SUPPLYTCR4c.m SUPPLYTCR5c.m loadTCR1 loadTCR2 loadTCR3 loadTCR4 TI12max loadTCR5 TI13max TI15max TI52max TI23max TI34max windTCR1 windTCR2 windTCR3 windTCR4 windTCR5
...
execute_unload "results.gdx"  gap gLevel U HourlyCost vSOC.l vCharge.l gVC sDischarge sCharge sSOC
