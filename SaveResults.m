...
% change back when running full year
% original: numDays = size(totalResults.date,2)-1;
...
% if (settings.wind == 2)
%     outputsDaily.wind = totalResults.wind;
%end
if (settings.isEVanalysis == 1)
...
if (settings.MDPM10 == 3),     outputsTotal.gMDPM10 = tMDPM10; end
 if (settings.wind == 3)
     outputsTotal.wind = sum(totalResults.wind);
end
